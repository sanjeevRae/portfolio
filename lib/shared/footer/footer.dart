...
class _FooterState extends State<Footer> {
  ...
  void _updateTime() {
    ...
    final kathmanduTime = utcNow.add(const Duration(hours: 5, minutes: 45));
    final formatter = DateFormat('h:mm a');
    setState(() {
      _kathmanduTime = formatter.format(kathmanduTime);
    });
  }
  ...
  Widget _buildDesktopLayout(BuildContext context) => Row(
    ...
    children: [
      Text(
        '© ${DateTime.now().year} Sanjeev.',
        style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.textPrimary.withValues(alpha: .7)),
      ),
      Text(
        'sanzeeprae@gmail.com',
        style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.textPrimary.withValues(alpha: .7)),
      ),
      Text(
        'Kathmandu $_kathmanduTime',
        style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.textPrimary.withValues(alpha: .7)),
      ),

      const Row(
        children: [
          _SocialIcon(icon: Icons.code, tooltip: 'GitHub', url: 'https://github.com/sanjeevRae'),
          SizedBox(width: 16),
          _SocialIcon(icon: Icons.work, tooltip: 'LinkedIn', url: 'https://www.linkedin.com/in/san-rai/'),
          SizedBox(width: 16),
        ],
      ),
    ],
